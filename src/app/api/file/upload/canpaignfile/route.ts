...
const fileFormValidator = new FormObjectValidator("type","file","userID","campaignId");
...
export async function POST( request : NextRequest )
{
    ...
    const errors = fileFormValidator.validateFormParams(formData);
    if( errors.length > 0 )
...
function createFile( formData: FormData, type: number) : ModelFile
{
    ...
    file.campaign_id = formData.get("campaignId")!.valueOf() as number;
