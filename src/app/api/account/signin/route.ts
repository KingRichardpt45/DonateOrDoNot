...
export async function POST(request: NextRequest) {
    ...
    if (session != null && session.expires >= new Date()) {
        ...
        if (errors.length > 0) return NextResponse.json({errors}, {status: 422, statusText: "Invalid form fields."})

        //console.log(formData)

        const result = await userManager.signIn(formData.get("email")!.toString().trim(), formData.get("password")!.toString().trim())
